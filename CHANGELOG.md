...
- **`DisplayText` returns a `textId`** - capture the returned `int` to update or clear the same on-screen text later, with new color/outline/shadow/spacing/wrap styling params; the `Vector2D` layout ports (position/anchor/scale/pivot/shadowOffset) are set per axis (`positionX`/`positionY`, …) — a constant bakes the property, a runtime value wires its `X`/`Y` sub-port.
...
- **LSP: default values in gate hovers** - hovering a gate/builtin (e.g. `SpawnPrefab`, `Sweep`, `DisplayText`) now shows a table of its parameter and settings-menu defaults (`limit = 5`, `distance = 100`, `fontSize = 16`, …), read from the same source the emitter uses; enum defaults show the member name (`direction = X_Positive`), composite/color defaults render as their constructor or sRGB hex (`outlineColor = #181425`), and a `Vector2D` sub-port axis shows that axis of its parent default (`anchorY = ` → `0.5`). Hovering a named argument (`fontSize = `) shows that field's default too.
