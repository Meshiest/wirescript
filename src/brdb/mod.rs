...
use crate::brdb::{
    ...
    wrapper::schemas::GLOBAL_DATA_SOA,
};
...
mod test {
    ...
    use crate::brdb::{
        ...
        wrapper::{
            Brick, World,
            schemas::{BRICK_CHUNK_SOA, BRICK_COMPONENT_SOA},
        },
    };
    ...
    fn test() -> Result<(), BrdbError> {
        let path = PathBuf::from("./components.brdb");
        if !path.exists() {
            return Ok(());
        }
        let db = Brdb::open(path)?;
        ...
        let chunk_data = db.read_file("World/0/Bricks/Grids/1/Components/-1_0_0.mps")?;
