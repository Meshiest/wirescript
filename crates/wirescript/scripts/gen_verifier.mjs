...
// v4: extendedMath + rounding gates are float-output; their table text is the
// game's FormatText-rounded (3-decimal, thousands-grouped) render, so a value
// EQ against full-precision live math would false-FAIL — assert by rendered
// text like the composite floats above. (Bitwise gates are exact int → value EQ.)
const FLOAT_RESULT_SHORT_NAMES = new Set([
...
function usesRenderedTextAssert(short, outputVariant) {
  return COMPOSITE_VARIANTS.has(outputVariant)
    || COMPOSITE_CHAPTER_FLOAT_SHORT_NAMES.has(short)
    || FLOAT_RESULT_SHORT_NAMES.has(short);
}

// v4: unary prefix operators (no builtin call surface).
const UNARY_OP = { MathNegate: '-', BitwiseNOT: '~' };
...
const CALL_FN = {
  ...
  // v4: extendedMath / bitwise-fn / rounding builtins.
  MathSin: 'sin', MathCos: 'cos', MathTan: 'tan',
  MathAsin: 'asin', MathAcos: 'acos', MathAtan: 'atan',
  MathSinh: 'sinh', MathCosh: 'cosh', MathTanh: 'tanh',
  MathAsinh: 'asinh', MathAcosh: 'acosh', MathAtanh: 'atanh',
  MathExp: 'exp', MathLn: 'ln', MathSqrt: 'sqrt', MathAbs: 'abs', MathSign: 'sign',
  MathDegreesToRadians: 'Deg2Rad', MathRadiansToDegrees: 'Rad2Deg',
  MathAtan2: 'atan2', MathPow: 'pow', MathMin: 'min', MathMax: 'max',
  MathLogBase: 'log', MathClamp: 'clamp', MathModuloFloored: 'fmod',
  Round: 'round', Floor: 'floor', Ceil: 'ceil',
  BitwiseNAND: 'BitNand', BitwiseNOR: 'BitNor', BitwiseBitCount: 'BitCount',
};
...
export function generateVerifier(table) {
  ...
  out.push('/// Verifies data/gate_semantics.json.');
  ...
  out.push('');
  out.push('on grid {');
  out.push(`  PrintToConsole("VERIFIER v${table.probeVersion}")`);
  ...
  for (const { short, c, expected, renderTextAssert } of cases) {
    ...
    } else if (UNARY_OP[short]) {
      out.push(`  let got${n} = ${wrap(`${UNARY_OP[short]}Opaque(${lit(c.inputs[0])})`)}`);
